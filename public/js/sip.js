...
function formatRepo(emp) {
  ...
  var markup = "<div class='select2-result-repository clearfix'>" + "<div class='select2-result-repository__avatar'></div>" + "<div class='select2-result-repository__meta'>" + "<div class='select2-result-repository__title'>" + emp.name + "</div>";

  if (emp.company) {
    markup += "<div class='select2-result-repository__description'>" + emp.company + "</div>";
  }

  markup += "<div class='select2-result-repository__statistics'>" + "<div class='select2-result-repository__forks'>Documento " + emp.document + " </div>" + "</div>" + "</div></div>";
  return markup;
}
...
$('.select-employees').select2((_$$select = {
  ...
  cache: true
}, _defineProperty(_$$select, "placeholder", 'Procurar Funcionario'), _defineProperty(_$$select, "minimumInputLength", 3), _$$select));
...
$('.inputMoney').mask('000.000.000.000.000,00', {
  reverse: true
});
  ...
  if (value) {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        var dataResponse = response.data['response'];
        var dataResponseCoodenadas = response.data['coordenadas'];
...
var checkboxPermissions = $(".checkboxPermissions");
...
var checkboxUserFolderPermission = $('.changeUserPermission');
...
var selectClientAddress = $(".select-client-addresses");
var selectAddress = $("#select-address");
selectClientAddress.change(function () {
  var self = $(this);
  var route = self.data('search-addresses');
  var value = self.val();
  $.ajax({
    type: 'GET',
    ...
    success: function success(response) {
      ...
      $.each(response.data, function (idx, item) {
        ...
        var address = description + ', ' + street + ', ' + number + ' - ' + district + ', ' + city + ' - ' + zip;
...
var selectClientEmployees = $(".select-client-employees");
var selectEmployee = $("#select-employee");
$(document).on('change', '.select-client-employees', function () {
  var self = $(this);
  var route = self.data('search-employees');
  var selectEmployee = $(self.data('target'));
  var value = self.val();
  $.ajax({
    type: 'GET',
    url: route + '?param=' + value,
...
var selectClientEmails = $(".select-client-emails");
var selectEmail = $("#select-email");
$(document).on('change', '.select-client-emails', function () {
  var self = $(this);
  var route = self.data('search-emails'); //let selectEmail = $(self.data('target'));
...
var selectOccupations = $(".select-occupations");
...
selectOccupations.change(function () {
  ...
  var self = $(this);
  var route = self.data('search-occupations');
  var value = self.val();
  $.ajax({
    type: 'GET',
    url: route + '?param=' + value,
    async: true,
    success: function success(response) {
      var html = "<option value=''>Selecione um cargo</option>";
      occupation.html(""); //occupation.selectpicker('refresh');

      $.each(response.data, function (idx, item) {
        html += "<option value=" + item.uuid + ">" + item.name + "</option>";
      });
      occupation.append(html); //occupation.selectpicker('refresh');

      swal.close();
    }
  });
});
var selectClientDocuments = $(".select-client-documents");
var tableDocuments = $("#table-documents");
selectClientDocuments.change(function () {
  var self = $(this);
  var route = self.data('search-documents');
  var value = self.val();
  swal({
    title: 'Carregando',
...
$('#daterange').daterangepicker({
  ...
  $("#end").val(end.format('DD/MM/YYYY')); //console.log('New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')');
});
...
function initMap() {
  ...
  autocomplete.addListener('place_changed', function () {
    ...
    if (place.address_components) {
      ...
      $("#pac-container").parent().append("<p class='text-success'>O Endereço está correto!</p>"); // Get each component of the address from the place details,
...
$calendar.fullCalendar({
  ...
  eventClick: function eventClick(event, element, view) {
    window.swal({
      title: 'Em progresso...',
...
function popularModal(event) {
  $.ajax({
    ...
    error: function error(data) {//alert(data.message);
...
/***/ 1:
/*!***********************************!*\
  !*** multi ./resources/js/sip.js ***!
  \***********************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /var/www/resources/js/sip.js */"./resources/js/sip.js");
